...
cf_stream/cf_list) that drive the c0 API and CHECK against expected values. The output
...
def gen_list(cases):
    """list.json: each case is a bare record stream holding one record whose
    `record` entries are scalars (bytes) or lists (arrays of item bytes). Lists
    are read via c0_field_items/c0_next_item on the raw field; when `canonical`
    the record is rebuilt with c0_build_record + c0_build_list_field/c0_build_field
    and must reproduce `bytes` exactly."""
    L = ["static void cf_list(void) {"]
    for c in cases:
        L.append("  { /* %s */" % c["name"])
        bn, bl = arr(L, "    ", hexbytes(c["bytes"]))
        rec = c["record"]
        L.append("    { c0_group g = c0_table(%s, %d); c0_iter ri = c0_group_records(g);"
                 " c0_bytes rec, f;" % (bn, bl))
        L.append("      CHECK(c0_next_record(&ri,&rec));")
        L.append("      CHECK(cf_arity(rec)==%d);" % len(rec))
        for idx, e in enumerate(rec):
            if isinstance(e, list):
                L.append("      CHECK(cf_field_n(rec,%d,&f) && cf_item_count(f)==%d);"
                         % (idx, len(e)))
                for j, it in enumerate(e):
                    inm, il = arr(L, "      ", field_bytes(it))
                    L.append("      CHECK(cf_item_eq(f,%d,%s,%d));" % (j, inm, il))
            else:
                fn, fl = arr(L, "      ", field_bytes(e))
                L.append("      CHECK(cf_val_eq(rec,%d,%s,%d));" % (idx, fn, fl))
        L.append("      CHECK(!c0_next_record(&ri,&rec)); }")
        if c["canonical"]:
            L.append("    { c0_builder bld; c0_builder_init(&bld);")
            fn, fl = arr(L, "      ", field_bytes(rec[0]))
            L.append("      { c0_bytes F[] = {{%s,%d}}; c0_build_record(&bld,F,1); }" % (fn, fl))
            for e in rec[1:]:
                if isinstance(e, list):
                    if e:
                        items = [arr(L, "      ", field_bytes(it)) for it in e]
                        lst = ",".join("{%s,%d}" % (n, l) for n, l in items)
                        L.append("      { c0_bytes I[] = {%s}; c0_build_list_field(&bld,I,%d); }"
                                 % (lst, len(items)))
                    else:
                        L.append("      c0_build_list_field(&bld,(c0_bytes*)0,0);")
                else:
                    fn, fl = arr(L, "      ", field_bytes(e))
                    L.append("      c0_build_field(&bld,%s,%d);" % (fn, fl))
            L.append("      CHECK(c0_builder_status(&bld)==C0_BUILD_OK);")
            L.append("      { c0_bytes got=c0_builder_bytes(&bld);")
            L.append("        CHECK(got.len==%d && memcmp(got.ptr,%s,%d)==0);" % (bl, bn, bl))
            L.append("        CHECK(c0_canonical(got.ptr,got.len)); }")
            L.append("      c0_builder_free(&bld); }")
        L.append("  }")
    L.append("}")
